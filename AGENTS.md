...
- `GET /views` — JSON `[{ path, views }]`, every path's pageviews from the same umami table (`GROUP BY url_path`, `event_type = 1`, the `WEBSITE_ID` filter), most viewed first, `?limit=` default 200 and at most 1000; `GET /views?path=/x/` is one path as `{ path, views }`, `0` when unknown. Paths are normalised before use as keys (query and hash dropped, doubled and trailing slashes stripped, the root is `/`), so `/slug/` and `/slug` add up. The whole table is one query cached for `CACHE_SECONDS` and served stale on error like the total; a single path is a lookup in that map. `Cache-Control: public, max-age=<CACHE_SECONDS>`, CORS `*`, 503 `views unavailable` when the DB never answered. costafotiadis.com reads it at build time for the `N views` on every post and again in the browser to refresh it (added 2026-09-25).
...
| `MOCK_VIEWS` | unset | local dev with `MOCK_COUNT`: `/views` table as `/a/=12,/b=3` (paths normalised, repeats add up) |
